 ...
 * by Chris Swagler
 * purpose: to use either the big sound or small sound microphone modules to detect if the sound is above
 *          a certain threshold to illuminate an LED
 *          
 * Note: the difference between the small and big sound modules is the scale of values detected. 
 *       Accordingly, the small sound threshold variable should be lowered since it reads much smaller 
 *       values. The potentiometer can also be used to adjust sensitivity for either module
*/
// www.elegoo.com
// 2018.10.24
/*
...
*/
int  threshold = 290;         // minimum analog reading for LED to turn on - can be calibrated
...
void loop() {
  ...
  if (analogValue >= threshold)     // When the Sound Sensor sends signla, via voltage present, light LED13 (L)
  {
